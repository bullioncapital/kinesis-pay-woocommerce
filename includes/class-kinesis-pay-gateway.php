...
class Kinesis_Pay_Gateway extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        // Register settings script for testing API connection on admin payment settings page
        add_action('admin_enqueue_scripts', array($this, 'admin_enqueue_ajax_callback_script'));

        // Verify credentials when saving payment settings
        add_action('woocommerce_update_options_checkout_kinesis_pay_gateway', array($this, 'verify_credentials'));
    ...
    public function init_form_fields()
    {
        $prod_fields = array(
            ...
            'merchant_id' => array(
                'title' => $this->test_mode ? sprintf('<span style="color: #868E95;">%s</span>', __('Merchant ID', 'kinesis-pay-gateway')) : __('Merchant ID', 'kinesis-pay-gateway'),
                'type' => 'text',
                'description' => $this->test_mode ? __('Test mode is now enabled. This value will be ignored.', 'kinesis-pay-gateway') : sprintf('<span style="color: #017DE8; font-weight: 600;">%s</span> %s',  __('Merchant ID', 'kinesis-pay-gateway'), __('generated by Kinesis. This can be found on KMS ', 'kinesis-pay-gateway')) . '<a href="' . Kinesis_Pay_WooCommerce::get()->get_kms_base_url() . '/merchant/dashboard" target="_blank">' . __('Merchant', 'kinesis-pay-gateway') . '</a>' . __(' page.', 'kinesis-pay-gateway'),
                'desc_tip' => false,
                'custom_attributes' => $this->test_mode ? array('readonly' => 'readonly') : array(),
            ),
            'publishable_key' => array(
                'title' => $this->test_mode ? sprintf('<span style="color: #868E95;">%s</span>', __('Access Token', 'kinesis-pay-gateway')) : __('Access Token', 'kinesis-pay-gateway'),
                'type' => 'text',
                'description' => $this->test_mode ? __('Test mode is now enabled. This value will be ignored.', 'kinesis-pay-gateway') : sprintf('<span style="color: #017DE8; font-weight: 600;">%s</span> %s',  __('Access Token', 'kinesis-pay-gateway'), __('generated by Kinesis. This can be found on KMS ', 'kinesis-pay-gateway')) . '<a href="' . Kinesis_Pay_WooCommerce::get()->get_kms_base_url() . '/settings/account/merchant-api-keys" target="_blank">' . __('Merchant API Keys', 'kinesis-pay-gateway') . '</a>' . __(' page.', 'kinesis-pay-gateway'),
                'desc_tip' => false,
                'custom_attributes' => $this->test_mode ? array('readonly' => 'readonly') : array(),
            ),
            'private_key' => array(
                'title' => $this->test_mode ? sprintf('<span style="color: #868E95;">%s</span>', __('Secret Token', 'kinesis-pay-gateway')) : __('Secret Token', 'kinesis-pay-gateway'),
                'type' => 'password',
                'description' => $this->test_mode ? __('Test mode is now enabled. This value will be ignored.', 'kinesis-pay-gateway') : sprintf('<span style="color: #017DE8; font-weight: 600;">%s</span> %s',  __('Secret Token', 'kinesis-pay-gateway'), __('generated by Kinesis. This can be found on KMS ', 'kinesis-pay-gateway')) . '<a href="' . Kinesis_Pay_WooCommerce::get()->get_kms_base_url() . '/settings/account/merchant-api-keys" target="_blank">' . __('Merchant API Keys', 'kinesis-pay-gateway') . '</a>' . __(' page.', 'kinesis-pay-gateway'),
                'desc_tip' => false,
                'custom_attributes' => $this->test_mode ? array('readonly' => 'readonly') : array(),
            ),
        );
        $last_fields = array(
            'test_connection_action' => array(
                'title' => __('Test API connection', 'kinesis-pay-gateway'),
                'label' => 'Test Now',
                'type' => 'button',
                'description' => __('Verify Merchant ID, Access Token and Secret Token, and test API connection.', 'kinesis-pay-gateway'),
                'desc_tip' => false,
            ),
        ...
        if ($this->test_mode) {
            $test_fields = array(
                'test_merchant_id' => array(
                    'title' => __('QA Merchant ID', 'kinesis-pay-gateway'),
                    'type' => 'text',
                    'description' => sprintf('<span style="color: #FF003C; font-weight: 600;">%s</span> %s',  __('Test mode', 'kinesis-pay-gateway'), __('is now enabled. QA Merchant ID is being used.', 'kinesis-pay-gateway')),
                    'desc_tip' => false,
                ),
                'test_publishable_key' => array(
                    'title' => __('QA Access Token', 'kinesis-pay-gateway'),
                    'type' => 'text',
                    'description' => sprintf('<span style="color: #FF003C; font-weight: 600;">%s</span> %s',  __('Test mode', 'kinesis-pay-gateway'), __('is now enabled. QA Access Token is being used.', 'kinesis-pay-gateway')),
                    'desc_tip' => false,
                ),
                'test_private_key' => array(
                    'title' => __('QA Secret Token', 'kinesis-pay-gateway'),
                    'type' => 'password',
                    'description' => sprintf('<span style="color: #FF003C; font-weight: 600;">%s</span> %s',  __('Test mode', 'kinesis-pay-gateway'), __('is now enabled. QA Secret Token is being used.', 'kinesis-pay-gateway')),
                    'desc_tip' => false,
                ),
            );
            $this->form_fields = array_merge($prod_fields, $test_fields, $last_fields);
        } else {
            $this->form_fields = array_merge($prod_fields, $last_fields);
        }
    }
...
?>
    ...
    /**
     * Generate test connection button HTML on admin settings page
     *
     * @param  string $key
     * @param  array $data
     * @return string
            ...
            <td class="forminp">
                <fieldset>
                    ...
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <span id="connection-message-success" style="color: #53A653;" hidden><?php echo esc_attr(__('Connected to Kinesis Pay API successfully.', 'kinesis-pay-gateway')); ?></span>
                        <span id="connection-message-failure" style="color: #FF003C;" hidden></span>
                    </div>
...
<?php
    ...
    /**
     * Verify Merchant ID, Access Token and Secret Token. Disable the gateway if verification fails.
     *
     * @return void
     */
    public function verify_credentials()
    {
        ...
        try {
            request_test_connection($this->merchant_id, $this->publishable_key, $this->private_key);
        } catch (Exception $e) {
            $this->update_option('enabled', 'no');
            WC_Admin_Settings::add_error(__('Failed to connect to Kinesis Pay API. ' . $e->getMessage(), 'woocommerce'));
            WC_Admin_Settings::add_error(__('The payment gateway has been disabled. Please check Merchant ID, Access Token and Secret Token.', 'woocommerce'));
        }
    }

    /**
     * Load kpay-setting.js on admin KPay settings page
     *
     * @return void
     */
    public function admin_enqueue_ajax_callback_script()
    {
        $screen = get_current_screen();
        if ($screen->base === 'woocommerce_page_wc-settings' && isset($_GET['page']) && $_GET['page'] === 'wc-settings' && isset($_GET['section']) && $_GET['section'] === 'kinesis_pay_gateway') {
            wp_enqueue_script('kpay-admin-settings', KINESIS_PAY_DIR_URL . 'assets/js/admin/kpay-settings.min.js');
            wp_localize_script(
                'kpay-admin-settings',
                'kpay_settings_data',
                array(
                    'is_test_mode' => $this->test_mode,
                    'test_connection_action' => 'test_connection',
                    'messages' => array(
                        'general_error' => __('Something went wrong. Please try again.', 'kinesis-pay-gateway'),
                        'failure_error' => __('Unable to connect to Kinesis Pay API.', 'kinesis-pay-gateway'),
                        'exception_error' => __('Something went wrong.', 'kinesis-pay-gateway'),
                        'input_missing_error' => __('Merchant ID, Access Token and Secret Token can not be empty.', 'kinesis-pay-gateway'),
                    ),
                )
            );
        }
